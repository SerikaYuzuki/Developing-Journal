# Developing-Journal 自動同期 (autosync)

複数のMacからこのリポジトリを使うために、launchdで定期的に
...
```sh
cd /Users/recky/GitHub/Developing-Journal
./scripts/install-autosync.sh
```

- `~/Library/LaunchAgents/com.serikayuzuki.developing-journal-autosync.plist` を生成し、
  `launchctl bootstrap` でロードする。
- 5分 (300秒) 間隔で実行され、ロード直後にも1回実行される (`RunAtLoad`)。
- 既にロード済みの場合は自動的に `bootout` してから再ロードするため、
  何度実行しても安全（冪等）。
- ログは `~/Library/Logs/developing-journal-autosync.log` に出力される。
...
```sh
launchctl bootout gui/$(id -u)/com.serikayuzuki.developing-journal-autosync
rm -f ~/Library/LaunchAgents/com.serikayuzuki.developing-journal-autosync.plist
```
...
- launchd実行時の標準出力・標準エラー: `~/Library/Logs/developing-journal-autosync.log`
