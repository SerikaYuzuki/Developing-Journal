...
各問では、最初に問題だけを読み、解き終えてから折りたたみの「解答」で式の流れを確認できます。問題文や解答が未掲載の箇所は、資料を受領後に追記します。

## 第1問

#### 問題

> 問題文は元資料を受領後に追記します。下図と既存の計算メモのみ掲載しています。
...
正電荷の要素が作る電場は電荷要素から観測点へ向かう。$\lambda(\phi)=\lambda_0\cos\phi$ は符号も含むので、全周を積分すると $y$ 成分は打ち消し合い、**$x$ 軸方向成分だけが残る**。以下ではその大きさを求める。
...
$y$ 成分と $z$ 成分は、$\phi$ と $\phi+\pi$ の電荷要素を組にすると打ち消し合う。残る $x$ 成分を考えると
$$
dE
= \frac{1}{4\pi\varepsilon_0}
...
$$
|E_x(z)|
\approx \boxed{\frac{\lambda_0 R^2}{4\varepsilon_0 z^3}}
$$

全電荷は $\int_0^{2\pi}\lambda_0R\cos\phi\,\dd\phi=0$ なので、遠方で $1/z^2$ の単極子項は現れず、$1/z^3$ で減衰する結果とも整合する。
