...
$a^2+1$ が素数とならないように $a$ を選ぶ。この時、$a^2+1= pq$
と1より大きい整数 $p,q$ が存在する。そうすると、

$$
a^2+1 = \left(\frac{p+q}{2} \right)^2 + \left(\frac{p-q}{2} \right)^2
$$

と表すことができる。ここで $n=\left(\frac{p+q}{2} \right)^2 - 1$
とすると、
...
となり、条件を満たす。このような $a$ は無限に存在するので、$n$
も無限に存在する。
...
となることから、 $a_i < 0$ となる。これを繰り返すことで、 $a_n < 0$
となる。よって、 $b_{2n-1} = 2a_0a_n > 0$
となり、矛盾する。ゆえに、答えは $2n-2$ 以下である。
...
考えている立方体の体積を $m^3$ とすると、 $m^3 = 3n^2 + 3n + 7$ から
$m\equiv 1 \pmod{3}$ となる。よって、 $m=3k+1$ とおけば、
...
となるわけだが、この右辺は $3$
で割り切れないが、右辺は割り切れて矛盾する。よって、このような立方体は存在しない。
