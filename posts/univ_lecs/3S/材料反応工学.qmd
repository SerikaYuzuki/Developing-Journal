...
を満たし、解析すると以下の関係が得られる：

$$
\frac{\nu}{\beta}
\exp\!\Bigl(-\frac{E_{\rm des}}{R\,T_{\rm max}}\Bigr)
= \frac{E_{\rm des}}{R\,T_{\rm max}^2}
\tag{★}
$$

Redhead (1962) は式(★)を近似して解析解を導出した：

1. $\ln\bigl(\tfrac{E_{\rm des}}{R\,T_{\rm max}^2}\bigr)$ 項を無視  
2. $\displaystyle \exp\!\Bigl(\tfrac{E_{\rm des}}{R\,T_{\rm max}}\Bigr)\approx \tfrac{\nu}{\beta}$  
3. 両辺の対数を取って整理すると  
...
\ln\!\Bigl(\frac{\nu}{\beta}\Bigr)
$$
4. 経験的補正 $-3.64$ を加えた最終式：

$$
E_{\rm des}
\approx R\,T_{\rm max}
\Bigl[\ln\!\Bigl(\tfrac{\nu\,T_{\rm max}}{\beta}\Bigr)
         \;-\;3.64\Bigr]
$$
