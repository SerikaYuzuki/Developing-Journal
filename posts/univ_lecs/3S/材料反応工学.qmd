...
\ce{A + B2 -> AB} \quad &r_3 = k_3[\ce{A}][\ce{B2}] \\
\end{aligned}
$$
...
\begin{aligned}
K &= \frac{[\ce{B2}]}{[\ce{B}]^2} \\
r_{\mathrm{total}} &= k_2[\ce{A}][\ce{B2}] \\
&= Kk_1[\ce{B}]^2[\ce{A}]
\end{aligned}
$$
...
$$
\begin{aligned}
\dv{[\ce{B2}]}{t} &= k_1[\ce{B}]^2 -k_2[\ce{B2}] - k_3[\ce{A}][\ce{B2}] + k_4[\ce{AB}] = 0\\
[\ce{B2}] &= \frac{k_1[\ce{B}]^2 + k_4[\ce{AB}]}{k_2 + k_3[\ce{A}]} \\
r_{\mathrm{total}} &= k_2[\ce{A}][\ce{B2}] \\
&= \frac{k_1k_2[\ce{A}][\ce{B}]^2 + k_1k_4[\ce{A}][\ce{AB}]}{k_2 + k_3[\ce{A}]}
\end{aligned}
$$
...
熱力学的鎖長の逆数が $[S]/[M]$ に比例することを示せ。ただし、[S] は開始剤濃度、[M] はモノマー濃度とする。
...
これらの反応における正味の生成速度と平衡時の濃度関係を示す図を描き、反応速度式を導出せよ。
