...
### 統合演習A：複素数・写像・正則性 {#math2f-basic-exercises}
...
### 統合演習B：Taylor 展開・Laurent 展開 {#math2f-series-exercises}
...
### 統合演習C：複素積分と実定積分 {#math2f-contour-exercises}
...
### 統合演習D：Fourier 級数と Fourier 変換 {#math2f-fourier-exercises}
...
### 統合演習E：Laplace 変換と微分方程式 {#math2f-laplace-exercises}
...
#### 最大値の原理
**証明**
$|f(z_0)|$ が真の極大値をとると仮定すると、開近傍で $|f(z_0)|>|f(z)|$。本編で導いた平均値公式へ代入すると

$$
|f(z_0)|\le\frac{1}{2\pi}\int_0^{2\pi}|f(z_0+re^{i\theta})|\,\dd \theta<|f(z_0)|
$$

となり矛盾する。したがって非定数正則関数の絶対値は領域内部で最大値を取らない。
...
**証明**
折れ線経路による原始関数 $F(z)=\int_{z_0}^z f(w)\,\dd w$ を構成できる。微分の定義より $F'(z)=f(z)$ となり、$F$ が正則。正則関数の導関数 $f$ も正則である。
...
#### ジョルダンの補題
**定理**
上半円弧 $C_{0,R}^+$ 上で最大値 $M_R \to 0$ $(R\to\infty)$ なら、$\int_{C_{0,R}^+} f(z)e^{i\eta z}\,\dd z \to 0$。
...
#### 偏角の原理とルーシェの定理
**証明**
偏角の原理：$\frac{1}{2\pi i}\oint_C\frac{f'(z)}{f(z)}\,\dd z=\sum p_i-\sum q_j$。$p_i$ 位の零点の近傍で $f(z)=(z-a_i)^{p_i}g(z)$ とおくと $f'/f=p_i/(z-a_i)+g'/g$ となり留数が $p_i$ となる。極についても留数が $-q_j$ となるため。
...
**証明の概要**
- **ガンマ関数の正則性:** $\Gamma(z) = \int_0^\infty t^{z-1}e^{-t}\dd t$ について、積分区間を分割し、指数関数の減衰によって一様収束性（モレラの定理）を用いて正則性を示す。
