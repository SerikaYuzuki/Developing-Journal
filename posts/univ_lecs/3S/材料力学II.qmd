...
このノートは、複数年度の過去問を同じ内容ごとにまとめ直したものです。各問は、まず問題だけを解き、必要になったら折りたたみの「理論」を読み、最後に「解答」で答案を確認できる順にしています。

図は、低解像度の資料画像を数値データとして読み取ったものではありません。設問の幾何と解析式から再作図した模式図です。OCRで確定できない箇所は末尾の「OCR・原資料の要確認箇所」にまとめています。
...
2. $\sigma_{11}$ のみが作用するとき、$\epsilon_{11},\epsilon_{22},\epsilon_{33}$ を求めよ。
3. $\sigma_{11},\sigma_{22},\sigma_{33}$ が同時に作用するとき、$\epsilon_{11}$ を求めよ。
4. $\sigma_{12}=2\mu\epsilon_{12}$ を用い、$E,\nu,\mu$ の関係を示せ。
...
![片持ち梁のせん断力、曲げモーメント、正規化たわみを並べた解析式による図。](images/材料力学II/cantilever-vmd-solution.svg){#fig-mech2-cantilever-diagrams fig-alt="上段は全区間でマイナスP一定のせん断力図、中段は固定端でマイナスPLから自由端でゼロへ直線的に変化する曲げモーメント図、下段は固定端で変位と傾きがゼロ、自由端で最大となる下向きたわみ曲線。" width="100%"}
...
![中央部等分布荷重を受けるロールのせん断力、曲げモーメント、正規化たわみを並べた解析式による図。](images/材料力学II/rolling-roll-vmd-solution.svg){#fig-mech2-rolling-diagrams fig-alt="上段は左右の無荷重区間で一定、中央荷重区間で直線的に減るせん断力図。中段は支点でゼロ、中央で最大となり、荷重区間で放物線となる曲げモーメント図。下段は左右対称で中央が最大の下向きたわみ模式曲線。" width="100%"}
...
転位線が $x_3$ 軸上にある、十分に長いらせん転位を考える。$x_1$ 軸から $x_3$ 軸のまわりを図の時計回りに1周するとき、$x_1,x_2$ 方向のずれはなく、$x_3$ 方向にだけ $b$ の変位が生じる。材料の剛性率を $\mu$ とする。

1. 弾性ひずみの全成分 $\epsilon_{ij}$ を求めよ。
2. 弾性応力の全成分 $\sigma_{ij}$ を求めよ。
3. 単位体積当たりの弾性ひずみエネルギーを求めよ。ただし、転位芯半径 $r_0$ から外側半径 $R$ までの弾性変形を考える。
...
であり、$\sigma_{13}=\sigma_{23}=0$ とする。

1. 転位Iの応力場が転位IIに及ぼす力を求めよ。
2. その $x_1$ 方向成分を $x_1$ の関数として図示し、2本の転位が安定して存在する位置を示せ。

::: {.callout-warning}
転記された原式の $\sigma_{33}$ は正号でしたが、上の $\sigma_{11},\sigma_{22}$ と平面ひずみ条件を組み合わせると負号になります。この設問の力には $\sigma_{33}$ を使わないため、結論には影響しません。詳しくは末尾のOCR確認欄に示します。
:::

::: {.callout-note collapse="true"}
##### 理論C2：応力そのものではなく、転位を滑らせる成分を取り出す

応力場の中に置かれた転位が受ける Peach--Koehler 力は、転位線の**単位長さ当たり**
...
なお、資料中の「転位間距離 $d$」という表現は図と整合しません。$d$ は二つのすべり面の間隔 $x_2=d$ であり、実際の転位間距離は $\sqrt{x^2+d^2}$ です。
:::
...
![刃状転位IIに働く無次元滑り力を位置x1/dに対して示し、同符号と異符号で安定点が入れ替わることを示す解析式によるグラフ。](images/材料力学II/edge-dislocation-force.svg){#fig-mech2-edge-force fig-alt="横軸x1/d、縦軸を規格化した滑り力。奇関数でx1/d=-1、0、1を横切る。同符号では0が安定で±1が不安定、異符号ではその逆。" width="96%"}
...
![観察面上の引張方向と主すべり面の交線として現れるすべり線、および両者の角度を示す模式図。](images/材料力学II/fcc-slip-trace.svg){#fig-mech2-fcc-slip-trace fig-alt="観察面バー301を正面から見た図。縦の引張方向[123]と、斜めのすべり線[1バー23]のなす鋭角をθ約64.6度と示す。結晶方位から描いた模式図。" width="78%"}
...
::: {.callout-warning}
以下はOCR文と添付図を数式上の整合性から点検した結果です。元の試験問題PDFを確認できていないため、「確定した訂正」ではなく、再確認が必要な箇所として残します。

1. **らせん転位のエネルギーの単位**  
   「単位体積当たり」と $r_0$ から $R$ までの積分指定は両立しません。局所密度は

   $$
   w(r)=\frac{\mu b^2}{8\pi^2r^2},
   $$

   積分して得る標準的な量は転位線の単位長さ当たり

   $$
   \frac{U}{\ell}
   =\frac{\mu b^2}{4\pi}\ln\frac{R}{r_0}
   $$

   です。本文では両方に答えました。

2. **刃状転位の $\sigma_{33}$ の符号**  
   OCR転記では
   ...
   でした。しかし、同じ問題に与えられた $\sigma_{11},\sigma_{22}$ と平面ひずみ条件
   ...
   となります。先頭の負号の脱落が疑われます。Peach--Koehler 力の計算では $\sigma_{11},\sigma_{12}$ しか使わないため、本問の答えは影響を受けません。

3. **転位IIの位置と $d$**  
   OCR文の「転位Iから距離 $d$ だけ離れて位置（図中の点線上）を滑る」は文が欠けています。図から読み取れる条件は「転位IIは $x_2=d$ の点線上を滑る」です。実際の転位間距離は $\sqrt{x_1^2+d^2}$ で、一定ではありません。

4. **刃状転位へ働く $F$ の意味**  
   無限に長い転位へ Peach--Koehler 式を使って直接得るのは、転位線の単位長さ当たりの力 $\boldsymbol f$ です。全力 $\boldsymbol F$ を求めるには有限な転位長さ $\ell$ が必要で、$\boldsymbol F=\ell\boldsymbol f$ です。

5. **安定点はバーガースベクトルの符号に依存**  
   元文では $b_{\mathrm I}b_{\mathrm{II}}$ の符号が明示されていません。同符号なら $x_1=0$、異符号なら $x_1=\pm d$ が、点線上の滑り運動に関する安定点です。

6. **圧延ロールの $q$ の次元**  
   文章では「圧力 $q$」、図では「等分布荷重 $q$」です。梁モデルで使う $q$ は線荷重 $\mathrm{N\,m^{-1}}$ です。接触圧力 $\mathrm{Pa}$ を意図した場合は、梁へ集約する奥行き幅が追加で必要です。

7. **らせん転位の角度の正方向**  
   添付図と文章は、$x_1$ から $x_2$ へ紙面上で時計回りに $\theta$ を測っています。通常の平面図で使う反時計回り角と取り違えると、ひずみ・応力の符号がすべて反転します。本文では図の向きを正と定義しました。符号付き $b$ を逆向きに定義しても同じ反転が起こり、エネルギーは不変です。

8. **片持ち梁の長さ記号**  
   OCR文は大文字 $L$、添付図は小文字 $l$ または筆記体の $\ell$ に見えます。本文では圧延ロールの $l$ と区別し、片持ち梁を $L$ に統一しました。

9. **余白の崩れた数式**  
   OCRに含まれた $b_8,\beta,\rho,J_1,9\pi$ などの式は、らせん転位の手書き解答
   ...
   2\pi r\,\dd r
   =
   \frac{\mu b^2\ell}{4\pi}
   \ln\frac{R}{r_0}
   $$

   が崩れた可能性があります。一方、$P/(KL)$ や $c^7/6^7$ を含む式は転位問題と対応せず、梁問題の別の手書き計算が混入したように見えます。いずれも問題条件としては採用しませんでした。

10. **科目名**  
    添付図には試験表紙や科目名が写っていません。記事名「材料力学II」は提供されたOCR見出しに従っていますが、転位論・結晶塑性を含むことから、元PDFでの確認余地があります。
:::
...
この記事の図は、次のスクリプトで解析式または設問の幾何から再生成できます。

```bash
uv run python posts/univ_lecs/3S/scripts/materials_mechanics_ii_figures.py
```
