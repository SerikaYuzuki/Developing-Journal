...
これは「電子がどこから来たか」を数える式ではなく、**結晶全体の正負の電荷を帳簿のように数える式**である。左辺は負電荷で、自由電子 $n$ は $-q$、電子を一個受け取って負にイオン化したアクセプタ $N_\mathrm{A}^-$ も一個あたり $-q$ を持つ。右辺は正電荷で、正孔 $p$ は $+q$、電子を手放して正にイオン化したドナー $N_\mathrm{D}^+$ も一個あたり $+q$ を持つ。巨視的な結晶が電荷を帯びると強い電場が生じてすぐ打ち消されるため、平衡では両者が等しい。

式を並べ替えると

$$
\boxed{n=p+N_\mathrm{D}^+-N_\mathrm{A}^-}.
$$

すなわち自由電子の数は、「正孔と正に帯電したドナーが作る正電荷」を打ち消すのに必要な数から、すでに負電荷を持つアクセプタの分を引いた数になる。ただし伝導帯へ入った電子を「Si由来」「ドナー由来」と区別して数えることはできない。ドナーが放した電子も、伝導帯に入れば同じ自由電子 $n$ である。
