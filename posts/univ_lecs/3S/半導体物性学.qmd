...
このノートは、2008年度から2024年度までの過去問を年度順に並べ直したものではない。**初めて半導体を学ぶ人が、必要な道具を一つずつ増やせる順**に、重複する問題を一つへ統合した対策ノートである。
...
元資料はOCR転記なので、数式コマンドの崩れや誤植を含む。本文では、重複年度と物理的整合性から復元できる式を正規化した。2014年以降の一部は問題要約しかなく、特に「あ〜し」の穴埋め本文が欠けているため、復元不能な箇所は推測で埋めず「資料欠損」と明示する。全年度の原問と統合先は末尾の[対応表](#semiconductor-source-map)から逆引きできる。
:::
...
| 部 | 中心となる問い | 身につける道具 | 主な出題年度 |
|---|---|---|---|
| 第1部 半導体をバンドで見る | なぜ物質によって電気の流れやすさが違うか | 伝導度、バンド、バンドギャップ、直接・間接遷移 | 2013、2014 |
| 第2部 電子の居場所を数える | 利用できる状態と電子の占有確率はいくつか | 状態密度、Fermi 分布、Boltzmann 近似、$n_i$ | 2008、2013、2018、2021、2024 |
| 第3部 不純物でキャリヤを作る | ドーピングで何が変わるか | Bohr 模型、ドナー・アクセプタ、Fermi 準位、温度依存性 | 2008、2009、2013、2014、2018、2022、2024 |
| 第4部 キャリヤはどう動くか | 電界・濃度差・再結合で流れはどう決まるか | 移動度、ドリフト、拡散、連続の式、拡散長 | 2009、2014、2016 |
| 第5部 pn接合 | 接合した瞬間と電圧印加後に何が起こるか | 拡散電位、空乏層、Poisson 方程式、I--V、C--V | 2008--2024の多くの年度 |
| 第6部 金属・半導体接合 | 整流接触と低抵抗接触は何で決まるか | 仕事関数、Schottky 障壁、Ohmic 接触 | 2008、2009、2012、2018 |
| 第7部 異種半導体デバイス | 材料を組み合わせると何が良くなるか | 格子整合、バンドオフセット、光閉じ込め | 2008、2009、2011、2012、2013、2016、2018、2021、2022、2024 |
| 第8部 集積回路素子 | トランジスタとメモリはどう働くか | BJT、HEMT、MOSFET、CMOS、DRAM、微細化 | 2012、2013、2014、2016、2018、2021、2024 |

この表で場所を確認したら、本文は第1部から順に読む。試験直前には、各問題の解答だけを閉じたまま再現し、最後の過去問対応表で未確認の年度がないかを点検する。
...
**出題対応：** 2014年度【1】の図の読み方。導体・半導体・絶縁体をバンドで説明する部分は [問題A2](#semiconductor-band-classification)、伝導度を変える要因は [問題D1](#semiconductor-conductivity-factors) に統合した。
...
![2014年度試験資料として提供された、絶縁体・半導体・導体の伝導度と比抵抗の範囲図。](images/半導体物性学/conductivity-chart-2014.png){#fig-conductivity-source fig-alt="下側の対数軸に伝導度、上側の逆向き対数軸に比抵抗を取り、石英、ダイヤモンド、Si、Ge、GaAs、金属などの範囲を示す日本語の資料図。" width="95%"}

上図は提供された試験資料である。次のPython図は、逆数軸と三分類の読み方を見失わないよう、同じ関係を模式的に再構成したもの。
...
**出題対応：** 2014年度【1】(1)(2)。2014年度【1】(3) は [問題D1](#semiconductor-conductivity-factors) で扱う。
...
**出題対応：** 2013年度問5。格子定数--バンドギャップ図を用いる2008年度問1、2009年度問1、2016年度【1】、2022年度【2】と、LED・半導体レーザ問題を解くための基礎理論もここへ統合した。
...
**統合元：** 2008問2、2013問1、2016【2】、2018問1、2021問1、2022【1】、2024問1
...
**統合元：** 2008問2、2013問1、2014【2】、2016【2】、2018問1、2021問1、2022【1】、2024問1

1. $np=n_\mathrm{i}^2$ と $n_\mathrm{i}$ の式を導け。
2. バンドギャップ $E_\mathrm{g}$ が大きいほど真性キャリヤ密度と電気伝導度がどうなるか述べよ。
3. 真性フェルミ準位 $E_\mathrm{i}$ を求め、300 K の Si について禁制帯中央からのずれを計算せよ。試験資料の $m_\mathrm{n}^*/m_0=0.26$、$m_\mathrm{p}^*/m_0=0.69$ を使う場合と、$N_\mathrm{c}=2.86\times10^{19}$、$N_\mathrm{v}=2.66\times10^{19}\ \mathrm{cm^{-3}}$ を使う場合を区別せよ。
...
単一の等方的放物線バンドで $N_\mathrm{c}\propto(m_\mathrm{n}^*)^{3/2}$、$N_\mathrm{v}\propto(m_\mathrm{p}^*)^{3/2}$ と置くなら

$$
E_\mathrm{i}-\frac{E_\mathrm{c}+E_\mathrm{v}}{2}
=\frac{3}{4}k_\mathrm{B}T\ln\left(\frac{m_\mathrm{p}^*}{m_\mathrm{n}^*}\right).
$$

重い側は同じエネルギー幅により多くの席を持つ。正孔側の席が多ければ、電子数と正孔数を等しくするため $E_\mathrm{i}$ は少し上へ動く。

::: {.callout-warning}
##### 提供定数には二つの近似が混在している

300 K で $k_\mathrm{B}T=0.0259\ \mathrm{eV}$ とする。

- 有効質量 $0.26m_0,0.69m_0$ を単一放物線モデルへ入れると、中央から **$+18.9\ \mathrm{meV}$（伝導帯側）**。
- 掲載された $N_\mathrm{c},N_\mathrm{v}$ を直接使うと、**$-0.94\ \mathrm{meV}$（価電子帯側）**。

Si の伝導帯には谷の縮退と異方性があり、「運動に使う有効質量」と「状態密度有効質量」は一般に同じではない。試験で使用値が指定されたら、その組だけを一貫して使い、採用モデルを一行添える。
:::
:::
...
\qquad
\boxed{n_\mathrm{i}=\sqrt{N_\mathrm{c}N_\mathrm{v}}
...
$$
\boxed{E_\mathrm{i}=\frac{E_\mathrm{c}+E_\mathrm{v}}2+\frac{k_\mathrm{B}T}{2}\ln\frac{N_\mathrm{v}}{N_\mathrm{c}}}.
$$

試験資料の有効質量を単一放物線モデルに使えば

$$
\Delta E_\mathrm{i}=\frac34(0.0259)\ln\frac{0.69}{0.26}
=\boxed{+1.89\times10^{-2}\ \mathrm{eV}}.
$$

資料掲載の $N_\mathrm{c},N_\mathrm{v}$ を直接使う場合は

$$
\Delta E_\mathrm{i}=\frac{0.0259}{2}\ln\frac{2.66}{2.86}
=\boxed{-9.4\times10^{-4}\ \mathrm{eV}}.
$$
:::
...
**統合元：** 2008問2・問3、2009問4、2013問2、2014【2】、2018問2、2024問2
...
**統合元：** 2008問2、2009問4、2011問1、2012問1、2013問2、2014【2】、2018問2、2021問2、2022【1】、2024問2

300 K のSiで $n_\mathrm{i}=9.65\times10^9\ \mathrm{cm^{-3}}$、$N_\mathrm{c}=2.86\times10^{19}\ \mathrm{cm^{-3}}$、$N_\mathrm{v}=2.66\times10^{19}\ \mathrm{cm^{-3}}$ とする。完全電離・非縮退を仮定する。

1. $N_\mathrm{A}=10^{17}\ \mathrm{cm^{-3}}$ のp型Siと、$N_\mathrm{D}=10^{18}\ \mathrm{cm^{-3}}$ のn型Siについて $n,p$ を求めよ。
2. p型では $E_\mathrm{F}-E_\mathrm{v}$、n型では $E_\mathrm{c}-E_\mathrm{F}$ を求めよ。
...
- p型（$N_\mathrm{A}=10^{17}$）：

  $$
  p\simeq\boxed{1.00\times10^{17}},
  \qquad
  n=\frac{n_\mathrm{i}^2}{p}=\boxed{9.31\times10^2}\ \mathrm{cm^{-3}}.
  $$

  $$
  E_\mathrm{F}-E_\mathrm{v}=k_\mathrm{B}T\ln\frac{N_\mathrm{v}}{N_\mathrm{A}}
  =\boxed{0.144\ \mathrm{eV}}.
  $$

- n型（$N_\mathrm{D}=10^{18}$）：

  $$
  n\simeq\boxed{1.00\times10^{18}},
  \qquad
  p=\frac{n_\mathrm{i}^2}{n}=\boxed{9.31\times10^1}\ \mathrm{cm^{-3}}.
  $$

  $$
  E_\mathrm{c}-E_\mathrm{F}=k_\mathrm{B}T\ln\frac{N_\mathrm{c}}{N_\mathrm{D}}
  =\boxed{0.0868\ \mathrm{eV}}.
  $$
...
**統合元：** 2009問3、2013問2、2014【2】、2018問2、2021問1、2022【1】
...
**出題対応：** 2014年度【1】(3)。伝導度の図と逆数関係は [問題A1](#semiconductor-conductivity-chart) に統合した。
...
**出題対応：** 2009年度問2。2014年度【3】(3)--(6)および2016年度【3】の理想ダイオード導出で再利用する、少数キャリヤ拡散方程式の基礎を兼ねる。
...
**対応年度:** 2008 問2、2009 問4、2011 問1、2012 問1、2013 問3、2014【2】、2016【2】、2018 問3、2021 問2、2022【1】、2024 問2
...
とする。B を $N_A=10^{17}\ \mathrm{cm^{-3}}$ 添加した p 型 Si と、As を
$N_D=10^{18}\ \mathrm{cm^{-3}}$ 添加した n 型 Si がある。完全電離・非縮退・無補償を仮定して、次を求めよ。
...
4. 過去問に現れる $N_A=10^{16},N_D=10^{17}$ および
   $N_A=10^{17},N_D=10^{15}\ \mathrm{cm^{-3}}$ の場合も計算せよ。
...
$$
p_p\simeq N_A=1.00\times10^{17}\ \mathrm{cm^{-3}},
$$
...
$$
n_n\simeq N_D=1.00\times10^{18}\ \mathrm{cm^{-3}},
$$
...
| $N_A\ (\mathrm{cm^{-3}})$ | $N_D\ (\mathrm{cm^{-3}})$ | 対応年度 | $V_{\mathrm{bi}}\ (\mathrm V)$ |
|---:|---:|:---|---:|
| $10^{16}$ | $10^{17}$ | 2008 問2 | $0.776$ |
| $10^{17}$ | $10^{18}$ | 2009、2011、2012、2013、2021、2024 | $0.896$ |
| $10^{18}$ | $10^{17}$ | 2014、2022 | $0.896$ |
| $10^{17}$ | $10^{15}$ | 2018 問3 | $0.717$ |
...
| 空欄 | 復元 |
|:---:|:---|
| A | $E_g$ が大きいほどキャリヤ密度は**小さい** |
| B | 電気伝導度は**小さい（低い）** |
| C | n 側から p 側へ移動するのは**電子**（同時に正孔は逆向きへ移動） |
| D | **空乏層** |
| E | 不純物濃度が低い側ほど**厚い** |
| F | 逆バイアスでは p 側に**負極** |
| G | 逆バイアスでは n 側に**正極** |
...
$$
(\text{か})\quad
\frac{kT}{2}\ln\frac{N_v}{N_c}
=\frac{3}{4}kT\ln\frac{m_h^*}{m_e^*},
$$

$$
(\text{き})\quad kT\ln\frac{N_c}{N_D},\qquad
(\text{く})\quad kT\ln\frac{N_D}{n_i},
$$

$$
(\text{け})\quad kT\ln\frac{N_v}{N_A},\qquad
(\text{こ})\quad kT\ln\frac{N_A}{n_i},
$$
...
である。$(\text{か})$ の二つ目の等号は、単一の放物線バンドで状態密度有効質量を使う場合に成り立つ。

**2014【2】は元の文章が資料中に残っていないため、同年が要求する $(\text{し})$ は安全には復元できない。** 原本なしに推測して穴を埋めない。
:::
...
**対応年度:** 2008 問2、2011 問1(7)、2012 問1(5)(6)、2013 問3(3)(4)、2014【3】(8)〜(10)、2016【3】、2018 問3、2021 問2(7)、2024 問2(6)
...
4. 300 K、$N_A=10^{17},N_D=10^{18}\ \mathrm{cm^{-3}}$、無バイアスの Si について $W,x_p,x_n$ および最大電場を求めよ。
...
#| fig-cap: 'NA=10^17 cm^-3、ND=10^18 cm^-3 の急峻 Si pn 接合における空間電荷密度、電場、電位、バンド端。横軸は接合界面からの距離で、灰色の範囲が空乏層である。'
#| fig-alt: 'p側の広い負電荷領域とn側の狭い正電荷領域、三角形状の負の電場、単調に上昇する電位、水平なフェルミ準位と曲がる伝導帯・価電子帯を縦に4段で示した図。'
#| code-fold: true
...
$$
\boxed{x_p=0.1035\ \mu\mathrm m},\qquad
\boxed{x_n=0.01035\ \mu\mathrm m}.
$$

p 側が n 側の10倍厚い。最大電場の大きさは
...
**対応年度:** 2008 問2(6)、2009 問4(6)、2011 問1(8)、2012 問1(7)、2013 問3(5)、2014【3】(11)、2016【3】、2018 問3、2021 問2(8)、2024 問2(7)
...
**対応年度:** 2008 問2(8)、2009 問4(7)、2011 問1(9)、2012 問1(8)、2013 問3(6)、2014【3】(1)〜(7)、2016【3】、2018 問3、2021 問2(9)、2022【1】
...
**対応年度:** 2016【2】、2018 問3、2021 問2(10)、2022【1】、2024 問2(8)
...
**対応年度:** 2008 問2(7)、2009 問5、2012 問5、2018 問2
...
**統合元：** 2008問1、2009問1、2011問3、2012問3、2016【1】、2022【2】
...
**統合元：** 2008問1、2009問1、2011問2、2012問2、2013問4、2014【4】、2016【1】、2018問4、2021問3、2022【2】、2024問3
...
**統合元：** 2008問1、2009問1、2011問3、2012問3、2013問4、2014【4】、2016【1】、2018問4、2021問3、2022【2】、2024問3
...
**統合元：** 2008問1、2009問1、2013問4、2014【5】、2016【1】【4】、2018問4、2021問3、2022【2】、2024問3
...
**統合元：** 2012問4、2013問4、2014【5】、2016【4】、2018問5、2021問3、2024問3
...
**統合元：** 2012問4、2014【5】、2016【4】、2018問5、2021問3、2024問3
...
## 過去問と統合問題の対応表 {#semiconductor-source-map}

同じ内容を年度ごとに繰り返す代わりに、次の統合問題IDへまとめる。完全に同じ文面だけでなく、数値や要求する導出の深さだけが異なる問題も同じIDに集約し、年度差は問題中の条件違いとして残す。

| 部 | 統合問題ID | 主題 |
|---|---|---|
| 第1部 | A1--A3 | 伝導度図、バンド分類、Si・Geと直接／間接遷移 |
| 第2部 | B1--B2 | 状態密度・Fermi分布、真性キャリヤと真性Fermi準位 |
| 第3部 | C1--C3 | Bohr模型・ドナー、ドープ半導体、三温度領域 |
| 第4部 | D1--D2 | 伝導度の要因、過剰少数キャリヤ拡散 |
| 第5部 | E1--E5 | pn接合平衡、空乏層、C--V、I--V、理想係数 |
| 第6部 | F1 | Schottky接合とオーミック接合 |
| 第7部 | G1--G4 | 材料図、太陽電池、LED・レーザ、BJT・HBT・HEMT |
| 第8部 | H1--H3 | MOSFET・CMOS、DRAM、微細化と新材料 |

「原資料行」は、転記元 `semiconductor_physics_exams.md` の行番号である。要約しか残っていない設問は、対応IDを「暫定」とした。

| 年度 | 原問 | 原資料行 | 統合問題ID | 対応する内容 |
|---:|---|---:|---|---|
| 2008 | 1(1) | 15--20 | G2 | タンデム太陽電池の材料選択と各層の役割 |
| 2008 | 1(2) | 15--21 | G3 | 半導体レーザの異種材料選択 |
| 2008 | 1(3) | 15--22 | G4 | ヘテロ接合BJTの材料選択 |
| 2008 | 2(1) A・B | 58, 94 | B2, D1 | バンドギャップ、真性キャリヤ密度、伝導度の関係 |
| 2008 | 2(1) C--G | 83--94 | E1, E2 | 接合形成、空乏層、幅の非対称性、順逆バイアス |
| 2008 | 2(2) あ--え | 29--48, 95 | B1 | 状態密度、Fermi分布、Boltzmann近似、電子・正孔密度 |
| 2008 | 2(2) お・か | 50--62, 95 | B2 | 真性キャリヤ密度と真性Fermi準位 |
| 2008 | 2(2) き--こ | 64--79, 95 | C2 | n型・p型半導体のFermi準位 |
| 2008 | 2(2) さ | 83--85, 95 | E1 | 拡散電位の式 |
| 2008 | 2(3) | 96 | B2 | 300 K Siの真性Fermi準位のずれ |
| 2008 | 2(4) | 97 | C1 | Asドナーの束縛エネルギー |
| 2008 | 2(5) | 98 | E1 | $N_D=10^{17},N_A=10^{16}\,\mathrm{cm^{-3}}$ の拡散電位 |
| 2008 | 2(6) | 99 | E3 | C--Vから不純物濃度・拡散電位を求める方法 |
| 2008 | 2(7) | 100 | F1 | n型半導体のSchottky条件とバンド図 |
| 2008 | 2(8) | 101 | E4 | 理想ダイオード式の導出 |
| 2008 | 3(1)--(3) | 105--116 | C1 | ボーア模型、準位、Rydberg定数の導出と計算 |
| 2009 | 1(1) | 150--155 | G2 | 2008年1(1)と同文 |
| 2009 | 1(2) | 150--156 | G3 | 2008年1(2)と同文 |
| 2009 | 1(3) | 150--157 | G4 | 2008年1(3)と同文 |
| 2009 | 2(1)(2) | 161--165 | D2 | 表面生成した過剰正孔の連続の式と濃度分布 |
| 2009 | 3 | 169--170 | C3 | n型電子濃度の三つの温度領域 |
| 2009 | 4(1) | 174--177 | C1 | ドナー準位のボーア模型計算 |
| 2009 | 4(2)(3) | 178--179 | C2 | p型・n型SiのFermi準位 |
| 2009 | 4(4) | 180 | E1 | 拡散電位の計算 |
| 2009 | 4(5) | 181 | E1 | 熱平衡でのFermi準位一致 |
| 2009 | 4(6) | 182 | E3 | C--V測定 |
| 2009 | 4(7) | 183 | E4 | 理想I--Vの導出 |
| 2009 | 5 | 187--188 | F1 | Schottky／オーミック条件と両バンド図 |
| 2010 | -- | -- | -- | 元資料に収録なし |
| 2011 | 1(1)(3) | 203--208 | C2 | p型・n型Siの多数・少数キャリヤ濃度 |
| 2011 | 1(2)(4) | 207--209 | C2 | p型・n型SiのFermi準位 |
| 2011 | 1(5) | 210 | E1 | 拡散電位 |
| 2011 | 1(6) | 211 | E1 | Fermi準位一致 |
| 2011 | 1(7) | 212 | E2 | 空乏層幅の比較 |
| 2011 | 1(8) | 213 | E3 | C--V測定 |
| 2011 | 1(9) | 214--219 | E4 | 理想I--Vの仮定と導出 |
| 2011 | 2(1)(2) | 223--227 | G2 | 太陽電池の原理・特徴・将来性 |
| 2011 | 2(3) | 228 | G2 | $V_{oc},I_{sc},FF$ |
| 2011 | 2(4) | 229 | G2 | 単接合の $E_g$ と効率 |
| 2011 | 2(5) | 230 | G2 | 多接合化と材料例 |
| 2011 | 3(1) | 234--237 | G3 | LEDの発光・波長制御・材料 |
| 2011 | 3(2) | 238 | G3 | LEDの特徴・将来性 |
| 2011 | 3(3) | 239 | G3 | LEDのヘテロ構造と高効率化 |
| 2011 | 3(4) | 240 | G3 | AlGaAs/GaAs/AlGaAsレーザとLEDとの差 |
| 2012 | 1(1)(2) | 253--257 | C2 | 2011年1(1)(3)と同文 |
| 2012 | 1(3) | 258 | E1 | 2011年1(5)と同文 |
| 2012 | 1(4) | 259 | E1 | 2011年1(6)と同文 |
| 2012 | 1(5) | 260 | E2 | 2011年1(7)と同文 |
| 2012 | 1(6) | 261 | E2 | 空乏層幅のバイアス依存性 |
| 2012 | 1(7) | 262 | E3 | C--Vによる濃度・拡散電位推定 |
| 2012 | 1(8) | 263 | E4 | 理想I--Vの前提と導出 |
| 2012 | 2(1) | 267--270 | G2 | 太陽電池の原理・特徴 |
| 2012 | 2(2) | 271 | G2 | $E_g$ と変換効率 |
| 2012 | 2(3) | 272 | G2 | タンデム太陽電池 |
| 2012 | 3(1) | 276--279 | G3 | LEDの発光・波長制御・材料 |
| 2012 | 3(2) | 280 | G3 | 2011年3(2)と同文 |
| 2012 | 3(3) | 281 | G3 | 半導体レーザとLEDとの差 |
| 2012 | 4(1) | 285--288 | H1 | MOSFETとCMOS NOT回路 |
| 2012 | 4(2) | 289 | H2 | DRAMセル |
| 2012 | 4(3)(4) | 290--291 | H3 | 微細化の効果と新材料 |
| 2012 | 5(1)(2) | 295--299 | F1 | 金属--半導体接合のI--V・形成条件 |
| 2013 | 1(1)--(3) | 312--321 | B1 | 状態密度、Fermi分布、$N_e,N_h$ |
| 2013 | 1(4) | 322 | B2 | 真性Fermi準位のずれ |
| 2013 | 2(1)(2) | 326--330 | C1 | ドナー準位の名称と束縛エネルギー |
| 2013 | 2(3) | 331 | C2 | As $10^{16}\,\mathrm{cm^{-3}}$ の電子・正孔濃度 |
| 2013 | 2(4) | 332 | C2 | Fermi準位とバンド図 |
| 2013 | 2(5) | 333 | C3 | 三温度領域 |
| 2013 | 3(1) | 337--340 | E1 | 拡散電位 |
| 2013 | 3(2) | 341 | E1 | Fermi準位一致 |
| 2013 | 3(3) | 342 | E2 | 空乏層幅の非対称性 |
| 2013 | 3(4) | 343 | E2 | 空乏層幅のバイアス依存性 |
| 2013 | 3(5) | 344 | E3 | C--V測定 |
| 2013 | 3(6) | 345 | E4 | 理想I--V |
| 2013 | 4(1) | 349--352 | G2 | 太陽電池の構造・原理・材料工学 |
| 2013 | 4(2) | 353 | G3 | 半導体レーザ |
| 2013 | 4(3) | 354 | G3 | LED |
| 2013 | 4(4) | 355 | G4 | BJT |
| 2013 | 4(5) | 356 | H1, H3 | MOSFET |
| 2013 | 4(6) | 357 | H2 | DRAM |
| 2013 | 5 | 361--362 | A3 | SiとGeの比較 |
| 2014 | 1(1)(2) | 374--380 | A2 | 導体・半導体・絶縁体のバンドによる比較 |
| 2014 | 1(3) | 381 | D1 | 伝導度を変える三要因 |
| 2014 | 2(1) | 385--388 | 暫定 C1, B1, C1, C2, E1 | あ--しの本文欠落につき厳密な内訳は不明 |
| 2014 | 2(2) | 389 | B2 | 真性Fermi準位のずれ |
| 2014 | 2(3) | 390 | C1 | Asドナーの束縛エネルギー |
| 2014 | 2(4) | 391 | C3 | n型電子濃度の温度依存性 |
| 2014 | 2(5) | 392 | C2 | As $10^{17}\,\mathrm{cm^{-3}}$ のFermi準位とバンド図 |
| 2014 | 2(6) | 393 | E1 | Fermi準位一定の証明 |
| 2014 | 2(7) | 394 | E1 | $N_A=10^{18},N_D=10^{17}\,\mathrm{cm^{-3}}$ の拡散電位 |
| 2014 | 3(1)--(7) | 398--407 | E4 | 仮定から少数キャリヤ分布・電流・理想I--Vまで |
| 2014 | 3(8)--(10) | 408--410 | E2 | Poisson方程式、電界、空乏層幅 |
| 2014 | 3(11) | 411 | E3 | C--V式 |
| 2014 | 4 | 415--416 | G2, G3 | LED・レーザ・単接合／タンデム太陽電池。小問内訳は欠落 |
| 2014 | 5 | 420--421 | G4, H1, H3 | BJT・MOSFET・微細化・新材料。小問内訳は欠落 |
| 2015 | -- | -- | -- | 元資料に収録なし |
| 2016 | 1(1) | 434--439 | G2 | 太陽電池の構造・原理・材料選択 |
| 2016 | 1(2) | 440 | G3 | 半導体レーザ |
| 2016 | 1(3) | 441 | G4 | HEMT |
| 2016 | 2 本体 | 445--446 | 暫定 C1, B1, C1, C2, C3, E1, E2 | 真性・n型・pn接合の理論式と計算。元の小問・穴埋め本文は欠落 |
| 2016 | 2 理想係数 | 448 | E5 | ηの1と2の違い |
| 2016 | 3 | 452--453 | E2, E3, E4 | I--V・C--Vの導出。小問内訳は欠落 |
| 2016 | 4 | 457--458 | G4, H1, H3 | BJT・MOSFET・材料工学。小問内訳は欠落 |
| 2017 | -- | -- | -- | 元資料に収録なし |
| 2018 | 1 | 471--472 | B1, B2 | 真性キャリヤ密度と真性Fermi準位 |
| 2018 | 2 第1項 | 476--477 | C1 | ドナー準位の概算 |
| 2018 | 2 第2項 | 478 | C2 | As $10^{16}\,\mathrm{cm^{-3}}$ のFermi準位とバンド図 |
| 2018 | 2 第3項 | 479 | C3 | 三温度領域 |
| 2018 | 2 第4項 | 480 | F1 | オーミック接合の条件とバンド図 |
| 2018 | 3 第1項 | 484--485 | E1 | $N_A=10^{17},N_D=10^{15}\,\mathrm{cm^{-3}}$ の拡散電位 |
| 2018 | 3 第2項 | 486 | E1, E2 | Fermi準位一致と空乏層幅の非対称性 |
| 2018 | 3 第3項 | 487 | E2, E3 | 空乏層幅とC--V |
| 2018 | 3 第4項 | 488 | E4, E5 | 理想I--Vと再結合電流などの非理想性 |
| 2018 | 4 | 492--493 | G2, G3, G4 | 太陽電池・レーザ・BJT。小問内訳は要約のみ |
| 2018 | 5 | 497--498 | H3 | MOSスケーリング、限界、新材料・プロセス |
| 2019 | -- | -- | -- | 元資料に収録なし |
| 2020 | -- | -- | -- | 元資料に収録なし |
| 2021 | 1 | 511--512 | B1, B2, C3 | 状態密度、Fermi分布、真性Fermi準位、温度依存性 |
| 2021 | 2(1)(3) | 516--520 | C2 | p型・n型Siのキャリヤ濃度 |
| 2021 | 2(2)(4) | 519--521 | C2 | p型・n型SiのFermi準位 |
| 2021 | 2(5) | 522 | E1 | 拡散電位 |
| 2021 | 2(6) | 523 | E1 | Fermi準位一致 |
| 2021 | 2(7) | 524 | E2 | 空乏層幅の比較 |
| 2021 | 2(8) | 525 | E3 | C--V測定 |
| 2021 | 2(9) | 526 | E4 | 理想I--V |
| 2021 | 2(10) | 527 | E5 | 理想係数 |
| 2021 | 3 | 531--532 | G2, G3, G4, H1, H3 | 太陽電池・レーザ・HEMT・MOSFET。個別の小問文は欠落 |
| 2022 | 1 穴埋め・真性半導体 | 545--546 | 暫定 C1, B1 | あ--しの本文欠落 |
| 2022 | 1 n型半導体 | 545--546 | C2, C3 | Fermi準位・バンド図・温度依存性 |
| 2022 | 1 pn接合 | 545--546 | E1, E4, E5 | 拡散電位、Fermi準位一致、I--V、理想係数 |
| 2022 | 2(1) | 550--554 | G2 | 太陽電池 |
| 2022 | 2(2) | 555 | G3 | 半導体レーザ |
| 2022 | 2(3) | 556 | G4 | HEMT |
| 2023 | -- | -- | -- | 元資料に収録なし |
| 2024 | 1(1)(2) | 569--572 | B1 | 状態密度と電子密度の導出 |
| 2024 | 1(3) | 573 | B2 | 真性Fermi準位のずれ |
| 2024 | 2(1) | 577--579 | C2 | p型Siのキャリヤ濃度 |
| 2024 | 2(2) | 580 | C2 | p型SiのFermi準位 |
| 2024 | 2(3) | 581 | C1 | ドナー準位 |
| 2024 | 2(4) | 582 | E1 | 拡散電位 |
| 2024 | 2(5) | 583 | E1 | Fermi準位一致 |
| 2024 | 2(6) | 584 | E2 | 空乏層幅の非対称性 |
| 2024 | 2(7) | 585 | E2, E3 | 空乏層容量のバイアス依存性 |
| 2024 | 2(8) | 586 | E5 | 理想係数 |
| 2024 | 3(1) | 590--592 | G2 | 太陽電池 |
| 2024 | 3(2) | 593 | G3 | 半導体レーザ |
| 2024 | 3(3) | 594 | G4 | BJT |
| 2024 | 3(4) | 595 | H1, H3 | ULSI用MOSFET |

::: {.callout-warning title="転記資料から確定できないこと"}
2014年以降は、原試験を一語ずつ転記したものではなく、設問全体を一行にまとめた年度が多い。特に2014年【2】、2016年【2】、2022年【1】は「あ〜し」に対応する文章そのものがないため、穴埋めの個数・順番・厳密な式は復元できない。上表の「暫定」は、残っている要約から学習先を割り当てたもので、原文一致を主張するものではない。

また、2014年【4】【5】、2016年【3】【4】、2018年の各問、2021年1・3、2022年1は小問内訳が省略されている。元PDFを確認できた場合は、そのPDFを優先して対応表を更新する。
:::

## このノートで用いる300 K Siの定数 {#semiconductor-constants}

次の値は、**2008年度問題の最終ページに転記されている数値を、このノート全体の共通規約として再利用したもの**である。他年度の「裏面の物性値」は資料に残っていないため、年度ごとの公式値を再現したものではない。TeXの制御文字だけを修復し、数値は変更していない。
...
::: {.callout-warning title="真性Fermi準位の数値に関する注意"}
掲載された有効質量を単一放物線模型へ使うと中央から $+18.9\ \mathrm{meV}$、掲載された $N_c,N_v$ を直接使うと $-0.94\ \mathrm{meV}$ となる。Siでは運動用有効質量と状態密度有効質量、谷の縮退を区別する必要があり、転記ミスとは断定できない。[問題B2](#semiconductor-intrinsic)では両方を明示した。答案では設問が指定したデータだけを一貫して使い、採用模型を一行書く。
:::

## 直前確認用公式集 {#semiconductor-formula-sheet}
